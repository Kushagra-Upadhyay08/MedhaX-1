...
function evaluateSingleAnswer(matchId, userId) {
  const match = activeMatches.get(matchId);
  if (!match) return null;
  ...
  if (!player._evaluatedQuestions) {
    player._evaluatedQuestions = new Set();
  }
  player._evaluatedQuestions.add(match.currentQuestionIndex);
